    ...
    def _weight_initialization(self):
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                ...
                raise ValueError('You should not init linear layers!')
...
class ResNet_cifar(ResNetBase):
    def __init__(
        ...
        self.freeze_bn = config['freeze_bn']
        self.freeze_bn_affine = config['freeze_bn_affine']
        ...
        self.save_activations = config['save_activations']
...
if __name__ == "__main__":
    ...
    import yaml
    sys.path.append("../../../")
    from src.utils import calculate_model_size
    yamlfile = '../../../experiments/Cifar100_Resnet8.yaml'
    with open(yamlfile, "r") as stream:
        config = yaml.load(stream, Loader=yaml.Loader)
    client_config = config['client_config']

    print("cifar100")
    net = ResNet_cifar(client_config)
    # print(net)
    mdict = net.state_dict()
    print(f"Resnet{client_config['resnet_size']}: size:{calculate_model_size(mdict):.3f} MB | num params:{sum(mdict[key].nelement() for key in mdict) / 1e6:.3f} M")
